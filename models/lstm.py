...
class LSTMModel:
    ...
    def build_model(self):
        ...
        model.add(LSTM(units=self.hidden_size, activation='relu', return_sequences=True, input_shape=(None, 1)))

        # Add remaining LSTM layers with dropout
        for _ in range(self.num_layers - 1):
            model.add(LSTM(units=self.hidden_size, activation='relu', return_sequences=True))
            model.add(Dropout(self.dropout))

        model.add(LSTM(units=self.hidden_size, activation='relu'))
        model.add(Dense(1))
